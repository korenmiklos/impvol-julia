...
\sum_{t=0}^{\infty} 
	...
	\right]
\]
subject to $\sum_j L_{njt}=L_{nt}$ and $w$ and $P$ being determined in equilibrium. (This includes adjustment costs)

The corresponding Bellman equation is
\begin{equation}
	V_n^N(\mathbf A_{t-1}) = 
	E V_n^D(\mathbf A_{t-1}, \mathbf A_{t})
\end{equation}
for the ``night'' period before productivity shocks have been realized. For the ``day'' period,
\begin{equation}
	V_n^D(\mathbf A_{t-1}, \mathbf A_{t}) = 
	\max_{\{L_{njt}\}}
		\ln \frac 
			{\sum_j w_{njt}(\mathbf A_{t}, \mathbf L_{t})L_{njt}}
			{P_{nt}(\mathbf A_{t}, \mathbf L_{t})}
...
\end{equation}
The day value depends on both time-$t$ and time-$t-1$ productivity because $\psi^*$ has been determined during the night period, before $A_t$ was known.
