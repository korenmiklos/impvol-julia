...
take place. Clearing in the input market within a sector implies 
\begin{equation*}
...
{aggregate}) by a CES formulation, 
\begin{equation}
C_{nt} = \left[
	\sum_{j=1}^{J}
		\left( \nu_{t}^{j}\right) ^{1/\sigma}
		\left( C_{nt}^{j}\right) ^{(\sigma-1)/\sigma}
	\right]^{\sigma/(\sigma-1)},
\label{ces_aggregate}
\end{equation}%
where $\sigma>0$ is the elasticity of substitution across sectors and $\nu_{t}^j$ is a demand shifter. We normalize $\sum_{j=1}^J \nu_t^j=1$. As in the Cobb--Douglas case, we let demand shifters vary over time.

This requires calibrating the $J$ demand parameters $\nu_{t}^j$ for each year, as well as the
elasticity of substitution $\sigma$. Our strategy is to calibrate the demand
